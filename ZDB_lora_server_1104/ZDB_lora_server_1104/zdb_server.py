...
class ZDB_Server(object):
    ...
    def unicast_common(self, auto_flag, gwnd, socket, cont_id, cont_cmd):
        ...
        l=tx.pro_cont_data_len
        #print(tx.pro_cont_data+tx.pro_cont_data_crc)
        if l != '05' and l != '06'  and l != '07' and l != '09' and l != '0D'and l != 'BE':
    ...
    def do_a_task_11(self, auto_flag, gwnd, socket):
        ...
        t = datetime.datetime.now()  # 将t=时间函数
        y = t.strftime('%Y%m%d%H%M%S')
        # year1 = (y[0:2])
        year2 = (y[2:4])
        month = (y[4:6])
        day = (y[6:8])
        ...
        year2 = hex(int(year2))
        year2 = year2.replace('0x', '')
        if len(year2) < 2:
            year2 = '0' + year2
        month = hex(int(month))
        month = month.replace('0x', '')
        if len(month) < 2:
            month = '0' + month
        day = hex(int(day))
        day = day.replace('0x', '')
        if len(day) < 2:
            day = '0' + day
        hour = hex(int(hour))
        hour = hour.replace('0x', '')
        if len(hour) < 2:
            hour = '0' + hour
        minute = hex(int(minute))
        minute = minute.replace('0x', '')
        if len(minute) < 2:
            minute = '0' + minute
        sec = (y[12:14])
        sec = hex(int(sec))
        ...
        if len(sec) < 2:
            sec = '0' + sec
        print('当前时间：',year2,month,day,hour,minute,sec)
        (cont_id,cont_cmd) = ('02','04'+'%02X'%value+year2+month+day+hour+minute+sec+'00')
        flag,res = self.unicast_common(auto_flag, gwnd, socket, cont_id, cont_cmd)
        if flag != 1:
