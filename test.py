...
checklist2 = ['liujunwei', 'yuanjun', 'zhanglili', 'zhangyonghui', '杨绵峰']
for p1 in checklist1:
    path = bpath + p1 + "/"
    print("当前目录下最新修改的目录为是：{}\n".format(sorted(Pt(path).items(),key=lambda d:d[1])[-1]))

for p2 in checklist2:
    path = bpath + p2 + "/"
    print("当前目录下最新修改的目录为是：{}\n".format(sorted(Pt(path).items(),key=lambda d:d[1])[-1]))

#print("当前目录下最新修改的目录为是：{}\n".format(sorted(Pt(r'//192.168.1.11/pubin/liujunwei/').items(),key=lambda d:d[1])[-1]))